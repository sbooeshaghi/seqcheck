...
const AUTH_CONFIG_ENV: &str = "SEQCHECK_AUTH_CONFIG";
const REMOTE_USER_AGENT: &str = "seqcheck/0.2";
...
fn config_location() -> ConfigLocation {
    ...
    if let Some(base) = env::var_os("XDG_CONFIG_HOME").map(PathBuf::from) {
        let path = base.join("seqcheck").join("auth.toml");
        return ConfigLocation {
            exists: path.exists(),
            path: Some(path),
    ...
    if let Some(home) = env::var_os("HOME").map(PathBuf::from) {
        let path = home.join(".config").join("seqcheck").join("auth.toml");
        return ConfigLocation {
            exists: path.exists(),
            path: Some(path),
...
hosts = ["127.0.0.1"]
kind = "basic"
username_env = "SEQCHECK_TEST_USER"
password_env = "SEQCHECK_TEST_PASS"
"#,
        );
        env::set_var(AUTH_CONFIG_ENV, &path);
        env::set_var("SEQCHECK_TEST_USER", "alice");
        env::set_var("SEQCHECK_TEST_PASS", "secret");
        ...
        env::remove_var(AUTH_CONFIG_ENV);
        env::remove_var("SEQCHECK_TEST_USER");
        env::remove_var("SEQCHECK_TEST_PASS");
...
"#,
    ...
    fn test_init_profile_creates_config_file() {
        ...
        let path = root.join("config").join("auth.toml");
        env::set_var(AUTH_CONFIG_ENV, &path);

        let output = init_profile(
            "igvf",
            AuthProfile {
                hosts: vec!["api.data.igvf.org".to_string(), "data.igvf.org".to_string()],
                kind: AuthKind::Basic,
                username_env: "IGVF_ACCESS_KEY_ID".to_string(),
                password_env: "IGVF_ACCESS_KEY_SECRET".to_string(),
            },
        )
        .unwrap();
        ...
        assert!(output.created_config);
        assert!(!output.updated_profile);
        assert!(path.exists());
        ...
        let registry = AuthRegistry {
            ...
            profiles: load_profiles(&ConfigLocation {
                path: Some(path.clone()),
                source: "test".to_string(),
                exists: true,
            })
...
"#,
        ...
        let profile = config.profiles.get("igvf").unwrap();
        assert_eq!(
            profile.hosts,
            vec!["api.data.igvf.org".to_string(), "data.igvf.org".to_string()]
        );
        assert_eq!(profile.username_env, "IGVF_ACCESS_KEY_ID");
        assert_eq!(profile.password_env, "IGVF_ACCESS_KEY_SECRET");
